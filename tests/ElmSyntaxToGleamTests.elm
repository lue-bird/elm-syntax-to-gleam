module ElmSyntaxToGleamTests exposing (suite)
...
suite : Test
suite =
    Test.describe "elm-syntax-to-gleam"
...
a2 x =
    case x of
        ...
        _ ->
            1
"""
                    |> expectTranspiledToGleamStringAs
                        """namespace global
...
expectTranspiledToGleamStringAs : String -> String -> Expect.Expectation
expectTranspiledToGleamStringAs expected source =
    case
                    ...
                    , declarations :
                        { valuesAndFunctions :
                            FastDict.Dict
                                ...
                                , result : ElmSyntaxToGleam.GleamExpression
                                , type_ : Maybe ElmSyntaxToGleam.GleamType
                                }
                        , typeAliases :
                            FastDict.Dict
                                String
                                { parameters : List String
                                , type_ : ElmSyntaxToGleam.GleamType
                                }
                        ...
                        , choiceTypes :
                            FastDict.Dict
                                String
                                { parameters : List String
                                , variants : FastDict.Dict String (List ElmSyntaxToGleam.GleamType)
                                }
                        }
                    }
                transpiledResult =
                    parsedModules |> ElmSyntaxToGleam.modules
            in
            case transpiledResult.errors of
                transpilationError0 :: transpilationError1Up ->
                    Expect.fail
                        ("failed to transpile the parsed elm to gleam: "
                            ++ ((transpilationError0 :: transpilationError1Up)
                                    |> String.join " and "
                               )
                        )
